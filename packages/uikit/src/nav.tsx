...
import { useBackButton, useBackIntercept } from "./telegram";

/* ---------------- Navigation stack (the genre differentiator) ---------------- */
...
export function TKNavStack({
  ...
  const commit = useCallback((next: NavEntry[]) => {
    setStack(next);
    changeRef.current?.(next.map((e) => e.panel));
  }, []);
  ...
  const api = useMemo<TKNavApi>(() => {
    ...
    return {
      ...
      popTo: (panel) => {
        const i = stackRef.current.findIndex((e) => e.panel === panel);
        if (i >= 0) commit(stackRef.current.slice(0, i + 1));
      },
      depth: stack.length,
  ...
  const handlers =
    swipeBack === "edge"
      ? {
          ...drag,
          onPointerDown: (e: Parameters<typeof drag.onPointerDown>[0]) => {
            const left = rootRef.current?.getBoundingClientRect().left ?? 0;
            if (e.clientX - left > EDGE_ZONE) return;
          ...
          return (
            <div
              ...
              style={{
                ...
                zIndex: i,
                ...(top && i > 0 && !dragging && !dragX ? { animation: "tk-nav-in var(--tk-t2) var(--tk-ease) both" } : null),
              }}
            >
              <TKNavContext.Provider
                value={top ? api : { ...api, activePanel: entry.panel, params: entry.params, depth: i + 1 }}
              >
