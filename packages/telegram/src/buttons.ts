 ...
 *
 * The provider owns the single `BackButton.onClick` and its visibility, driving
 * show/hide from the want-count that `useBackIntercept` maintains. This hook
 * only registers `onBack` as the top interceptor — it deliberately does NOT
 * call `show()`/`hide()` itself. A direct `hide()` on unmount would yank the
 * one native button out from under another still-mounted consumer that still
 * wants it (the back-button race); deferring to the want-count keeps the button
 * visible until the last interceptor leaves, and routes one press to one
 * handler.
 */
