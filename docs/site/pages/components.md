...
- **Select dropdowns share the contract.** The `TKSelect` and `TKMultiselect` option lists portal into the same host, glued to the trigger (re-measured on open, scroll, and resize), so a scrolling card or `overflow: hidden` wrapper no longer clips the popup. Keyboard navigation, Escape/outside-close, and the focus contract are unchanged; a dev warning fires when the host is not positioned and the dropdown cannot be placed reliably.
- **No invisible regional defaults.** `TKPhoneInput` no longer hardcodes `+7` with the Russian mask: the default derives from the active locale (`TKLocale.lang`, or the `lang` prop). A Russian locale keeps `+7` + the Russian grouping mask; any other (or no) locale makes the simple field a free unmasked international input (`numberMask` still masks the digits if given), and the `countrySelect` variant picks its country from the `lang` region subtag (`"en-US"` → US) with a documented US fallback. An explicit `defaultCountry` always wins.
- **Every built-in string is localizable.** The last hardcoded strings — the `TKDateInput`/`TKTimeInput` validation messages, the calendar's Month/Year part-selector labels, and the `TKTimeInput` AM/PM group name — resolve through `TKLocale` (`invalidDate`, `invalidTime`, `month`, `year`, `amPm`) with English fallbacks and shipped Russian translations; per-instance props (`invalidText`) still win.
