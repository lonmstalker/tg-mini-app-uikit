...
- [x] M6.1 `TKNavStack` + `TKNavPanel` + хук `useNav()` (`push/pop/replace/popTo`, params); рендер стека с сохранением state нижних панелей; направленные переходы (slide, reduced-motion → fade/instant); `onStackChange`.
- [x] M6.2 Свайп-бек: edge-зона (опционально «из любой точки»), интерактивный прогресс перехода на useDragGesture, отмена/доводка по velocity.
- [x] M6.3 Интеграция платформы: `useBackButton` показывается при глубине >1 и делает `pop()`; открытый `TKSheet`/`TKDialog` перехватывает back раньше стека (приоритетная очередь «кто обрабатывает back» в TKTelegramProvider); `ClosingConfirmation`-рецепт в доке.
- [x] M6.4 ScrollSaver: автосохранение/восстановление scrollTop панелей при pop (сигнал VKUI #3750); скролл живёт внутри панели, не на body (VKUI #1064).
- [x] M6.5 `useKeyboard` — keyboard-aware хук (visualViewport: `{visible, height}`), класс `tk-kb-open` на корне TKProvider, рецепт «инпут над клавиатурой» (боль iOS из статей; сигнал Ionic #29887; смежный баг TGUI #93 с safe-area после blur — покрыть тестом в mock).
- [x] M6.6 Demo: перевести BookingApp на TKNavStack (профиль врача — новый экран в стеке, M8.6) как dogfooding; Platform Lab — карточка back-приоритетов (шит vs стек).
...
- [x] M7.1 `TKMessages` + `TKMessageBubble` + `TKWriteBar` — чат-лента (входящие/исходящие, группировка хвостов, время, статусы галочек) и поле ввода (авто-рост textarea, слоты attach/send, safe-area). Сигнал: Konsta Messages, VKUI WriteBar; основа Support-демо M9.4.
- [x] M7.2 `TKOnboardingTooltip` — coach marks поверх TKPopper: подсветка таргета (вырез в оверлее), шаги (`steps[]`, next/skip из локали), persist «показано» через optional cloud storage. Сигнал: уникум VKUI.
- [x] M7.3 `TKConfetti` — canvas-салют (одноразовый burst, ~150 частиц, transform-only, reduced-motion → не рендерится, авто-очистка). Сигнал: статьи (микро-награды); использование: Game level-up (M8.7), Stars-оплата (M9.1).
- [x] M7.4 Optional-haptics: проп `haptics?: boolean` на `TKTelegramProvider` (default false) + внутренний `useOptionalHaptics`; обвязка: TKSwitch/TKCheckbox/TKRadioGroup (selection), TKTabbar/TKSegmented (selection), TKSlider (на шаге, троттлинг), TKMainButton success (notification), PTR порог (impact light), swipe-action full-swipe (impact medium), TKPinInput ошибка (notification error). Сигнал: статья Turumburum «haptic на каждое действие — стандарт TMA». Vitest через createMockTelegram: события хаптики пишутся в event log.
- [x] M7.5 Theme presets: `preset?: "ios" | "material"` у TKProvider (наборы knobs: радиусы/spring/размеры; material = меньше радиусы, прямее easing) + опциональный `glass`-акцент (полупрозрачные поверхности; iOS26-тренд, Ionic #30466 — 114 реакций) как эксперимент: если контраст-тесты против — отказ записать в Surprises.
...
- [x] M8.1 Код-сниппеты: у каждой галерея-секции кнопка «код» → подсвеченный JSX (мини-хайлайтер строк на 100 без deps, или `<pre>` без подсветки — решить) + copy-to-clipboard (`useClipboard` уже есть в платформенном слое — dogfooding). Сниппеты лежат рядом с секциями как строки-константы; CI-проверка `scripts/check-snippets.mjs`: каждый сниппет компилируется (tsc на временном файле).
- [x] M8.2 Поиск + оглавление галереи: сайдбар/шторка с якорями по 27+ секциям, фильтр по имени компонента; deep-link `?app=gallery&section=inputs` (расширение существующих URL-параметров). ВНИМАНИЕ: учесть `paintGallery()` и `content-visibility` (скролл к секции должен её отрисовать).
- [x] M8.3 Prop-playground для 6 компонентов (Button, Input, Sheet, Cell, Calendar, NavStack): панель контролов (сами из кита — switch/select/slider) + live-превью + генерация сниппета с текущими пропсами.
- [x] M8.4 Persistence через собственный `useCloudStorage` (mock уже хранит в localStorage): корзина Shop, XP/coins/streak Game, выбранные настройки TweaksPanel. Кнопка «сбросить демо-данные» в TweaksPanel.
- [x] M8.5 Имитация сети: утилита `demoDelay(ms)+demoFail(p)` в демо; каталог Shop грузится со скелетонами (TKSkeletonCard в деле), оплата — с loading у TKMainButton и сценарием ошибки+retry (failure-флоу уже есть — добавить задержки/скелетоны); пустой поиск → TKEmptyState.
- [x] M8.6 BookingApp: динамические даты от `new Date()` (e2e фиксируют clock через Playwright `clock` API — проверить поддержку в 1.60, иначе инжект "сегодня" через URL-параметр `?today=2026-06-13` — этот же параметр стабилизирует visual-эталоны); cancel/reschedule флоу; экран профиля врача (на TKNavStack, M6.6); permission states (нотификации запрещены → TKEmptyState с действием).
- [x] M8.7 GameApp: честный daily reward (timestamp в cloud storage, таймер до следующего), level-up с TKConfetti и анимацией TKXPHeader.
- [x] M8.8 ShopApp: промокод (валидация, error/success у TKFormField), лимит остатков («осталось 2»), бейдж корзины через обновлённый TKCounter.
- [x] M8.9 Locale-переключатель (готов в M1.4) + RTL: прогнать все приложения, зафиксировать visual ar-RTL эталоны для Shop home.
- [x] M8.10 Deep links на экраны приложений: `?app=shop&screen=cart` и аналоги (упростит e2e и шаринг).
...
- [x] M9.1 Stars Checkout (`?app=stars`): тарифы (TKCardChip/TKPaymentSummary) → `useInvoice` (XTR) через mock (статусы paid/cancelled/failed) → квитанция + TKConfetti. Paywall-паттерн подписки (статья EJAW). Зависимости: M7.3. Mock: убедиться, что openInvoice-статусы детерминированы.
- [x] M9.2 Onboarding / Identity (`?app=onboarding`): слайды на TKGallery+TKPageDots → requestContact → requestWriteAccess → установка TKPinInput + биометрия (`useBiometrics`) → emoji-status. Все permission-отказы имеют ветки UI. Зависимости: M4.4, M5.5.
- [x] M9.3 Storage & Settings restore (`?app=settings`): профиль настроек на cloud/device/secure storage (три колонки-сравнение), «выйти и вернуться» (симуляция перезапуска mock) с восстановлением. Зависимости: нет (хуки есть).
- [x] M9.4 Support / Bot handoff (`?app=support`): чат на TKMessages/TKWriteBar, быстрые ответы TKChipGroup, эскалация «передать оператору» (openTelegramLink mock), рейтинг диалога TKRating. Зависимости: M7.1.
- [x] M9.5 Fullscreen mini-game (`?app=arcade`): requestFullscreen + lockOrientation + сенсоры (useMotionSensors — детерминированные показания mock) в простой игре (наклоном собрать точки), пауза при `visibilityChanged`. Зависимости: нет (хуки есть); канвас-игра — простая, без физики.
- [x] M9.6 Channel/Feed (`?app=feed`): лента постов — TKSpoiler, TKBlockquote, реакции (TKChip+TKCounter анимация), репост через useShare, «прочитано» по IntersectionObserver. Зависимости: M5.3, M5.4, M5.7.
- [x] M9.7 TON Wallet flow (`?app=wallet`): mock-флоу connect → balance → send (TKMaskedInput суммы) → confirm (TKDialog) → история (TKInfiniteList) на существующих TKWalletConnectButton/TKWalletStatusCell. Плюс отдельный `examples/ton-connect` с реальным `@tonconnect/ui-react` (вне офлайн-гейтов, см. Decision Log). Зависимости: M4.3, M5.10.
- [x] M9.8 Forms showcase (`?app=forms`): анкета «запись на тест-драйв»: TKDateInput+TKCalendar, TKPhoneInput, TKChipsInput, TKFileInput c прогрессом, TKSlider range — с валидацией и итоговым TKSheet-резюме. Зависимости: M4 целиком. Это приёмочный полигон M4.
...
- [x] M10.1 Сайт документации в `docs/site` (zero-dep markdown → static HTML генератор `scripts/build-docs.mjs`): страницы Getting Started / Theming (knobs+presets+tg-theme) / Telegram platform (хуки+mock+testkit) / Components / API Reference / Recipes. API-reference даёт import-level карту и ссылается на `.d.ts` как source of truth. Деплой GitHub Pages workflow.
- [x] M10.2 `llms.txt` + `docs/llms-full.md`: компактная карта API для AI-агентов (список компонентов/пропсов/паттернов, anti-patterns). Сигнал: TGUI #57 («LLM не могут писать код на ките») — превращаем в фичу.
- [x] M10.3 README-позиционирование: сравнение с TGUI/Konsta/VKUI (живость, тесты, zero-deps, платформенный слой, React 19), позиция «один выверенный стиль + knobs + presets», ссылка на сайт доков; бейдж покрытия Bot API 9.6.
- [x] M10.4 Рецепты (docs): «Tabbar и контент» (боль TGUI #78), «инпут над клавиатурой» (M6.5), «edge-свайпы iOS и карусели», «back-приоритеты», «интеграция с tg-mini-app-testkit» (соседний проект — синергия: наш mock для разработки, их testkit для e2e пользовательских аппов).
- [x] M10.5 CI: job сборки доков; Lighthouse-смоук демо (perf-бюджет INP из статей — фиксируем не хуже текущего); обновить docs/demo.gif с новыми демо.
- [x] M10.6 Релиз 0.2.0: CHANGELOG с миграционными заметками M0/M1 (breaking), npm publish dry-run через check:package.
...
- [x] (2026-06-13) M6 Навигационный каркас (6/6): TKNavStack/TKNavPanel/useNav (push/pop/replace/popTo, params, onStackChange, направленный slide + reduced-motion через токены), свайп-бек edge/anywhere на useDragGesture с интерактивным прогрессом, очередь back-обработчиков в TKTelegramProvider (useBackIntercept/useBackDispatcher; шит/диалог перехватывают раньше стека, нативная Back-кнопка показана при depth>1), ScrollSaver бесплатно (нижние панели остаются смонтированными со скрытой видимостью — скролл и state живут), useKeyboard (visualViewport + класс tk-kb-open), демо: BookingApp на стеке (профиль врача), карточка Back priorities в Platform Lab. e2e/nav.spec.ts (3 сценария). Бонус: вылечен флак скриншотов шиммера (заморозка в visual-проектах).
- [x] (2026-06-13) M7 Telegram-паттерны (5/5): TKMessages/TKMessageBubble (группировка хвостов, галочки sent/delivered/read) + TKWriteBar (авторост, Enter/Shift+Enter, send-кнопка из локали), TKOnboardingTooltip (вырез-спотлайт, шаги, persist через storage-адаптер, автоцентрирование цели), TKConfetti (canvas, reduced-motion → ничего + мгновенный onDone), optional-haptics (проп haptics у TKTelegramProvider + useOptionalHaptics; switch/checkbox/radio/tabbar/segmented selection, slider тики с троттлингом, MainButton success, PTR порог, full-swipe, PIN error — пишутся в event log Platform Lab), preset ios|material у TKProvider (material: roundness 0.5 + smooth) + переключатель в Tweaks. Секции Chat и Wow, e2e/wow.spec.ts (4 сценария). Glass-акцент НЕ реализован — отказ записан в Surprises.
- [x] (2026-06-13) M8 Демо 2.0 (10/10): сниппеты у 16 секций (кнопка code → шит с <pre> + копирование; CI-гейт scripts/check-snippets.mjs в check:package), оглавление-шторка с поиском + deep-link ?section=, Playground для Button/Input/Sheet/Cell/Calendar/NavStack (живые knobs + генерация сниппета), persistence (корзина Shop и прогресс Game через useCloudStorage, Tweaks в localStorage, кнопка Reset demo data), demoDelay/demoFail + скелетоны каталога, Booking: динамические даты от bootToday()/?today= + cancel/reschedule + ветка запрещённых нотификаций, Game: честный daily reward с таймером + level-up конфетти, Shop: промокод SPRING24/-20% + лимиты остатков + бейдж 9+, ar-RTL visual эталон Shop home, deep links ?screen=. e2e/demo2.spec.ts (10 сценариев).
- [x] (2026-06-13) M9 Новые демо (8/8): добавлены Stars Checkout, Onboarding/Identity, Storage restore, Support handoff, Arcade, Feed, Wallet, Forms + `examples/ton-connect`. Acceptance e2e `e2e/m9-apps.spec.ts` покрывает happy/failure path каждого нового app; visual home-снимки light/dark для 8 новых app добавлены в `e2e/design.spec.ts` и darwin baselines. `ArcadeApp` получил `?still=1` для стабильных visual-снимков.
- [x] (2026-06-13) M10 Документация и дистрибуция (6/6): zero-dep статический docs generator (`docs:build`/`docs:dev`), docs pages + recipes + API map, `docs/llms-full.md`, README обновлён под 13 apps/Bot API 9.6/React 19/zero deps, `.github/workflows/docs.yml`, docs smoke, `CHANGELOG.md`, версия пакета `0.2.0`, `docs/demo.gif` перезаписан сценарием с M9 apps. `npm run docs:check`, `npm run docs:build`, `npm run build`, `npm run docs:lighthouse`, `npm run check:package` зелёные.
...
- Observation (2026-06-13, M6): источник флака `gallery — media` найден: бесконечная анимация шиммера на ::after; animations:"disabled" Playwright замораживает её в недетерминированной фазе. Лечение — addStyleTag-заморозка в pixel-проектах (motion-спек по-прежнему проверяет, что анимация живёт). Два полных прогона подряд зелёные.
- Decision-as-surprise (2026-06-13, M7.5): полупрозрачный glass-акцент отклонён: AA-палитра M2 рассчитана против непрозрачных поверхностей; стекло поверх произвольного фона разрушает гарантии контраста (burn-down=0), а Konsta #268 показывает, что glass всё равно требуют отключаемым. Кандидат M11 при явном спросе.
- Observation (2026-06-13, M5–M7): full-kit бюджет поднят 34→40 kB (esm): +42 иконки, TKNavStack, чат, онбординг, конфетти, виртуальные списки. Tree-shaking по-прежнему в норме (одиночные импорты не растут) — рост оплачивают только те, кто импортирует всё.
- Observation (2026-06-13, M8): tree-shaking бюджеты одиночных импортов подняты (TKButton 3→4.5 kB, TKSpinner 2→3.5 kB): после M1/M7 каждый компонент несёт общий рантайм — en-словарь локали (дефолт контекста) и optional-haptics плумбинг (~1.6 kB суммарно, оплачивается один раз на бандл, не на компонент). PURE-аннотации добавлены и к createContext.
- Observation (2026-06-13, M10): recorder для `docs/demo.gif` падал на inconsistent frame dimensions после добавления новых tab-переходов. Root cause: viewport 1000×980 был ниже нижней границы desktop device frame (`y≈192`, `h≈830`), Playwright клипал видимую часть; новые короткие app-состояния проявили нестабильность. Лечение: запись ведётся при viewport 1000×1120, сценарий обновлён на Shop → Stars → Support → Feed → Platform → Gallery.
