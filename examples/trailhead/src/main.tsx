...
import { TKTelegramProvider, tkResolveTelegramBridge } from "@tg-mini-app/telegram";
...
async function bootstrap() {
  /*
   * The kit owns the whole launch sequence (it took three real-device
   * iterations to get right — wiki/ios-debugging.md): load the vendored
   * bridge only when the host didn't provide one, classify by platform,
   * delete the outside-Telegram stub. Production must not silently turn into
   * a fake Telegram runtime: real Mini Apps get the official bridge,
   * local/dev previews get the injected mock, and production browser
   * fallback stays honest unless `?mock=1` is explicit.
   */
  const realBridge = await tkResolveTelegramBridge();
